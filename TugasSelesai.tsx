...
const TugasSelesai: React.FC = () => {
  ...
  const filteredTasks = tasks
    ...
    .filter(t=>{
      ...
      const dateStr = (()=>{ try{ return new Date(t.deadline).toISOString().slice(0,10);}catch{return '';} })();
  ...
  return (
    <div className="h-full rounded-lg bg-black p-4 text-white border border-purple-900 flex flex-col gap-2 overflow-auto">
      ...
      {filteredTasks.length===0 && <p className="text-gray-500 text-sm text-center">Tidak ada tugas selesai</p>}
      {filteredTasks
        .sort((a,b)=>{
